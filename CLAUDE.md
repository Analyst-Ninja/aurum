...
- CI (`.github/workflows/ci.yml`) runs on `main`, `develop`, `epic/*`, and PRs — ruff + pytest + Sonar. The install step is `uv sync --locked --no-build --group modeling`, because `tests/modeling/` imports the ML stack. `terraform.yml` has two path-filtered jobs over `infra/terraform/**`: `validate` (fmt/validate/tflint) on pushes and PRs, and `apply` on merge to `main` only (GH-84) — it assumes `aurum-github-actions` by OIDC (`infra/terraform/github_oidc.tf`, trust policy pinned to `ref:refs/heads/main`), reads `image_tag` off the live `aurum-ingest-market` task definition rather than the commit SHA, and serializes on a `terraform-apply` concurrency group because the S3 backend's `use_lockfile` fails a concurrent run. Building and pushing the image is still local (`make push`). See `docs/operations/cicd.md` §3.
