...
- CI (`.github/workflows/ci.yml`) runs on `main`, `develop`, `epic/*`, and PRs — ruff + pytest + Sonar. The install step is `uv sync --locked --no-build --group modeling`, because `tests/modeling/` imports the ML stack. `terraform.yml` has two jobs: `validate` (fmt/validate/tflint) on pushes and PRs, and `apply` on merge to `main` only (GH-84) — it assumes `aurum-github-actions` by OIDC (`infra/terraform/github_oidc.tf`, trust policy pinned to `ref:refs/heads/main`) and serializes on a `terraform-apply` concurrency group because the S3 backend's `use_lockfile` fails a concurrent run. **`apply` builds and pushes the image too**, so a merge that changes `src/` ships to Fargate with no local step: the push trigger covers `infra/terraform/**` plus everything the image contains (`src/**`, `docker/**`, `main.py`, `pyproject.toml`, `uv.lock`), and `image_tag` is the short SHA of the last commit that touched one of those image paths — not HEAD, so an infra-only commit resolves to the tag already deployed and rebuilds nothing. The job pushes that tag only if `aws ecr describe-images` does not already find it (the repo is `IMMUTABLE`, and the check is what makes a re-run idempotent). This replaced reading `image_tag` off the live task definition, which was a fixed point: the resolved value always equalled the deployed one, so `apply` reported `No changes` forever and code could never ship from CI. `make push` still works locally and is the rollback path (`-var="image_tag=<older sha>"`). See `docs/operations/cicd.md` §3.
