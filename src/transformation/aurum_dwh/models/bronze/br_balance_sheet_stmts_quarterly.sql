{#
    Typed, deduplicated mirror of public.balance_sheet_stmts_quarterly. Follows the shape of
    br_income_stmts_quarterly, the bronze reference model, changing only the
    source name and the period column (QTR).
...
source as (
    ...
    select * from {{ source('landing', 'balance_sheet_stmts_quarterly') }}
